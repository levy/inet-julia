...
worktree, step by step, ticking the checkboxes and recording decisions here as we go.
...
- [x] Record the baseline: `julia --project=. -e 'using Pkg; Pkg.test()'` — the numbers every
      ...
      **Baseline (2026-08-03, `main` at b536b33): 2094 pass / 0 fail / 0 error / 0 broken**
      — `packet & chunk API` 1680, `10BASE-T1S / PLCA` 414; ~19s wall, 913 MB peak.
      ...
      count must not.
- [ ] Create the implementation worktree **as a sibling**:
      `git worktree add /home/projectured/workspace/inet-julia-split -b component-package-split`.
- [ ] Confirm `queuing-wave1` is landed or explicitly deferred (§6).

### S1 — `package/packet` (`InetPacket` + test + example)

- [ ] `git mv src/packet/*.jl package/packet/main/` (13 files, flat).
- [ ] Add `package/packet/main/Project.toml` (`entryfile = "InetPacket.jl"`, no deps) and
      `InetPacket.jl`: `include("Packet.jl"); using .PacketModule; export PacketModule`.
- [ ] `git mv test/packet/*.jl package/packet/test/`; add `Project.toml`, `runtests.jl` and
      `InetPacketTest.jl` exposing `test_packet()`.
- [ ] `git mv examples/packet_api_demo.jl package/packet/example/`; add `Project.toml` +
      `InetPacketExample.jl`. Remove the now-empty `examples/`.
- [ ] `git mv documentation/packet.md package/packet/doc/`.
- [ ] Root `Project.toml`: add `InetPacket` to `[deps]` + `[sources]`; `src/Inet.jl` drops
      `include("packet/Packet.jl")` in favour of `using InetPacket: PacketModule`.
- [ ] Verify: `julia --project=package/packet/test -e 'using InetPacketTest; test_packet()'`
...
- [ ] `git mv src/t1s/*.jl package/linklayer/main/t1s/` and
      `git mv src/model/T1sModel.jl package/linklayer/main/t1s/`.
- [ ] Add `Project.toml` + `InetLinkLayer.jl` (includes `t1s/T1s.jl`, `t1s/T1sModel.jl`;
...
- [ ] `git mv test/t1s/* package/linklayer/test/` (including `inet-reference/notraffic.vec`);
      add `Project.toml`, `runtests.jl`, `InetLinkLayerTest.jl` with `test_linklayer()`.
- [ ] `git mv scripts/compare_t1s_vectors.jl package/linklayer/test/`; note in its header that
      it is a CLI tool and not part of `runtests.jl`. Remove the now-empty `scripts/`.
- [ ] `git mv documentation/ten-base-t1s.md package/linklayer/doc/`.
- [ ] Verify: `julia --project=package/linklayer/test -e 'using InetLinkLayerTest; test_linklayer()'`.

### S3 — `package/common` and `package/queuing` *(requires §6)*

- [ ] `git mv src/lookup/*.jl package/common/main/lookup/`; add `Project.toml` +
      `InetCommon.jl` (includes `lookup/Lookup.jl`, exports `LookupModule`).
- [ ] `git mv test/queuing/phase0_lookup.jl package/common/test/`; add the test package with
      `test_common()`.
- [ ] `git mv src/queuing/{contract,base,source,sink}/*.jl package/queuing/main/…`; the include
      list in `QueuingLayer.jl` becomes the body of `InetQueuing.jl` and the file is dropped.
- [ ] Rewrite the two cross-package imports: `using ..PacketModule` →
...
- [ ] `git mv test/queuing/* package/queuing/test/`; add the test package with `test_queuing()`.
- [ ] Verify: `julia --project=package/common/test -e 'using InetCommonTest; test_common()'` and
      the same for `InetQueuingTest`.

### S4 — `package/inet` (the umbrella)

- [ ] `git mv src/Inet.jl package/inet/main/` and `git mv src/model/Catalog.jl package/inet/main/`.
      Remove the now-empty `src/`.
- [ ] `Inet.jl` becomes the umbrella: `using` each component package, re-export
...
- [ ] Add `package/inet/{main,test}/Project.toml`; `InetTest.test_inet()` covers the catalog
      (`inet_simulation_catalog()` lists `T1sModel`) and is the home for future cross-component
      integration tests.
- [ ] Verify: `julia --project=package/inet/test -e 'using InetTest; test_inet()'`.

### S5 — the root becomes an environment

- [ ] Rewrite the root `Project.toml`: drop `name`/`uuid`/`version`/`[extras]`/`[targets]`,
...
- [ ] Write `test/runtests.jl` as the aggregator: `test_packet()`, `test_common()`,
      `test_queuing()`, `test_linklayer()`, `test_inet()` inside one `@testset "inet-julia"`,
      with the per-component commands in the header comment.
- [ ] Delete the stale root `Manifest.toml` and re-instantiate (`Manifest.toml` is gitignored;
      every environment resolves its own).
- [ ] Verify: `julia --project=. test/runtests.jl` totals the S0 baseline (2094 pass, 0 fail,
      0 error) once the queuing suite's own count is added on top.

### S6 — documentation

- [ ] Write `documentation/architecture.md`: the package graph and the dependency rule, what
...
- [ ] Rewrite the README's "Project layout" section as the package table, and update the
      getting-started commands (`--project=package/packet/example …`).
- [ ] Add `CLAUDE.md`: layout, the "run the smallest test that covers the change" rule with the
      per-component commands, the sibling-worktree constraint, and the plan convention.
- [ ] Update the path references inside `plan/pending/queuing-model-migration.md` (`src/queuing/`
...
- [ ] Reuse `ProjecturedKernelTest.check_layering(src_root, top_file)` in each component's test
      package so the slice DAG inside a package is checked statically, the way
      `projectured-julia` does per package. Costs a `ProjecturedKernelTest` dep in the test
      packages only.
- [ ] Confirm every package environment resolves standalone from a clean state:
...
- [ ] The repository root contains only `package/`, `documentation/`, `plan/`, `test/`,
      `Project.toml`, `README.md`, `CLAUDE.md`, `.gitignore`.
- [ ] `julia --project=. test/runtests.jl` reproduces the S0 baseline.
- [ ] Each of the five components runs its own suite in its own environment.
- [ ] `documentation/architecture.md` and the README describe the layout that exists.
- [ ] This plan moved to `plan/done/`.
