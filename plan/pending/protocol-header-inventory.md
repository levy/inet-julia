...
Status: **IN PROGRESS**. Phases 0 to 7 are done, Waves 1 to 3 are in — IEEE 802.11 included — and Wave 4 has begun. 214 wire formats are declared and every one round-trips. The repository is green:
...
**Done — 58 formats.** RIP, the eight AODV control packets, the DSDV hello, the
eighteen PIM formats, and BGP's header, KEEPALIVE, OPEN and NOTIFICATION with
its optional parameters, the seven RTP and RTCP formats, DHCP with its options, and the three application payloads.
...
| 9 ◐ | Wave 3 ✅, Wave 4 started — §3.7 | green over 214 formats |
