...
The dispatch entry that draws a packet as the figure. It is keyed on `Packet`,
not on `PacketDiagram`: a packet held anywhere in a viewed document draws as the
figure, with no conversion at the place that holds it.
"""
